...
比特币网络的原生货币作为功能货币和资产同时使用。当比特币作为工具时，可以用于支付交易费用，当作为资产时，可以用于保存长期的价值。这两类使用场景通常视为交易媒介(MoE)和价值媒介(SoV)。这两类场景并不冲突。它们对于网络都是非常重要的。学习这两类用户的经济动机将对分析比特币网络的长期可用性非常有帮助。

The Bitcoin protocol constrains the network's transaction throughput by using a fixed block size. Users bid with fees on the limited throughput to have their transactions processed. With this auction like mechanism, transaction fees are determined by the transaction demand - the more demand there is on the network, the higher the transaction fee a user has to pay to beat the competition and have their transaction included in the block.  
比特币协议通过固定的区块大小约束网络的交易量。用户通过手续费的竞价来让网络处理他们的交易。基于这种机制，交易费用将由交易需求决定。对于网络有越多的需求，用户愿意用来竞争和打包自身交易而支付的交易费越高。
